...
test_that("matrix (single zone)", {
  # simulate data
  pu <- data.frame(id = seq_len(10), cost = c(0.2, runif(9)),
                   spp1 = runif(10), spp2 = c(rpois(9, 4), NA))
  # create problem
...
test_that("matrix (multiple zones)", {
  ...
  y[is.na(as.matrix(pu[, c("cost_1", "cost_2")]))] <- 0
  # calculate representation
  r <- feature_representation(x, y)
  # create correct result
...
test_that("data.frame (multiple zone)", {
  ...
  y[[1]][is.na(pu$cost_1)] <- 0
  y[[2]][is.na(pu$cost_2)] <- 0
  # calculate representation
  r <- feature_representation(x, y)
  # create correct result
  pos <- which(!is.na(pu$cost_1) | !is.na(pu$cost_2))
...
test_that("Spatial (multiple zone)", {
  ...
  pu$s1[is.na(pu$cost_1)] <- 0
  pu$s2[is.na(pu$cost_2)] <- 0
  # create problem
  x <- problem(pu,
...
test_that("Raster (multiple zone)", {
  ...
  y[[1]][is.na(sim_pu_zones_stack[[1]])] <- 0
  y[[2]][is.na(sim_pu_zones_stack[[2]])] <- 0
  y[[3]][is.na(sim_pu_zones_stack[[3]])] <- 0
  # calculate representation
  r <- feature_representation(x, y)
  # create correct result
