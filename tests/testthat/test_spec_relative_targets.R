...
test_that("numeric (5)", {
  ...
  p <-
    problem(sim_pu_raster, sim_features) %>%
    add_auto_targets(method = spec_relative_targets(v))
  # calculate absolute targets
  targets <- p$targets$output(p)
  # run tests
  expect_inherits(targets, "tbl_df")
...
test_that("invalid inputs", {
  ...
  # run tests
  expect_tidy_error(
    spec_relative_targets(p, 0.1),
    "add_auto_targets"
  )
  expect_tidy_error(
    p %>% add_auto_targets(spec_relative_targets("a")) %>% compile(),
    "numeric"
  )
  expect_tidy_error(
    p %>% add_auto_targets(spec_relative_targets(c(0.1, 0.2))) %>% compile(),
    "length"
  )
  expect_tidy_error(
    p %>% add_auto_targets(spec_relative_targets(NA_real_)) %>% compile(),
    "missing"
  )
  expect_tidy_error(
    p %>% add_auto_targets(spec_relative_targets(-0.1)) %>% compile(),
    "between"
  )
  expect_tidy_error(
    p %>% add_auto_targets(spec_relative_targets(1.1)) %>% compile(),
    "between"
  )
})
