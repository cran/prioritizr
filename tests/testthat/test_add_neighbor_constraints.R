...
test_that("compile (manually data specified, single zone)", {
  ...
  o2 <- p %>% add_neighbor_constraints(1, data = a1) %>% compile()
  o3 <- p %>% add_neighbor_constraints(1, data = a2) %>% compile()
  o4 <- p %>% add_neighbor_constraints(1, data = a3) %>% compile()
  o5 <- p %>% add_neighbor_constraints(1, zones = NULL, data = a4) %>% compile()
...
test_that("compile (SpatRaster, multiple zones)", {
  # import data
