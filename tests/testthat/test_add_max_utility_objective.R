...
test_that("compile (expanded formulation, single zone)", {
  ...
  expect_equal(o$col_ids(), c(rep("pu", n_pu), rep("pu_ijz", n_pu * n_f),
                              rep("amount", n_f)))
  expect_equal(o$row_ids(), c(rep("pu_ijz", n_f * n_pu), rep("spp_amount", n_f),
                              "budget"))
...
test_that("compile (compressed formulation, multiple zones, scalar budget)", {
  ...
  expect_equal(o$col_ids(), c(rep("pu", n_pu * n_z),
                              rep("amount", n_f * n_z)))
  expect_equal(o$row_ids(), c(rep("spp_amount", n_f * n_z),
                              "budget", rep("pu_zone", n_pu)))
  expect_equal(o$lb(), rep(0, (n_pu * n_z) + (n_f * n_z)))
  expect_equal(o$ub(), c(rep(1, (n_pu * n_z)),
...
test_that("solve (compressed formulation, multiple zones, scalar budget)", {
  ...
  cost <- raster::stack(
    ...
    raster::raster(matrix(c(1,  2,  2,  NA, NA), nrow = 1)))
  features <- raster::stack(
    raster::raster(matrix(c(50, 50, 50, 50, 50), nrow = 1)),
    raster::raster(matrix(c(50, 50, 50, 50, 50), nrow = 1)),
    raster::raster(matrix(c(2,  1,  1,  0,  5), nrow = 1)),
    raster::raster(matrix(c(30, 30, 30, 30, 30), nrow = 1)))
  # create problem
  ...
  expect_equal(raster::values(s[[2]]), c(0, 1, 1, NA, NA))
})
...
test_that("solve (compressed formulation, multiple zones, vector budget)", {
  ...
  features <- raster::stack(
    raster::raster(matrix(c(50, 50, 50, 50, 50), nrow = 1)),
    raster::raster(matrix(c(1, 4, 50, 50, 50), nrow = 1)),
    raster::raster(matrix(c(6,  5,  5,  0,  11), nrow = 1)),
    raster::raster(matrix(c(30, 30, 30, 30, 30), nrow = 1)))
  # create problem
  p <- problem(cost, zones(features[[1:2]], features[[3:4]])) %>%
       add_max_utility_objective(budget = budget) %>%
