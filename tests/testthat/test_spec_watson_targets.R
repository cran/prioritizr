test_that("defaults", {
  # create object
  x <- spec_watson_targets()
  # tests
  expect_equal(x$args$rare_area_threshold, 10000)
  expect_equal(x$args$rare_relative_target, 1)
  expect_equal(x$args$rare_area_target, 1000)
  expect_equal(x$args$rare_method, "min")
...
test_that("raster features", {
  ...
  ## calculate targets
  correct_targets <- rep(NA_real_, length(fs))
  idx <- fs < 10000
  correct_targets[idx] <- pmin(1000, 1 *fs)
  idx <- fs > 10000
  correct_targets[idx] <- fs[idx] * 0.1
  ## apply target caps
