...
test_that("data.frame (solve, single zone)", {
  ...
  s <-
    ...
    add_proportion_decisions() %>%
    add_manual_locked_constraints(
      data.frame(pu = c(1, 90, 5, 2), status = c(0.1, 0.2, 0.3, 0.4))
    ) %>%
    add_default_solver(verbose = FALSE) %>%
    solve(run_checks = FALSE)
  # tests
  expect_equal(s$solution_1, c(0.1, NA, 0.2, 0.3, 0.4), tolerance = 1e-6)
})
...
test_that("sf (compile, multiple zones)", {
  ...
  expect_equal(o$ub()[locked_pos], c(rep(0.3, 5), 0.7))
  expect_true(all(o$lb()[other_pos] == 0))
  expect_true(all(o$ub()[other_pos] == 1))
})
...
test_that("data.frame (compile, multiple zones)", {
  ...
  sim_zones_pu_data$spp_1_zone_3 <- 2
  sim_zones_features <- get_sim_zones_features()
  # create problem
  p <-
    problem(
...
test_that("data.frame (solve, multiple zones)", {
  ...
  expect_equal(s$solution_1_zone_2, c(0, NA, 0, 0.7, 0), tolerance = 1e-5)
  expect_equal(s$solution_1_zone_3, c(1, NA, 1, 0.3, 0.9), tolerance = 1e-5)
})
...
test_that("invalid inputs (single zone)", {
  ...
  expect_tidy_error({
    add_manual_locked_constraints(
      p,
      data.frame(pu = seq_len(10) + 1000, status = 0.2)
    )
  })
  expect_tidy_error({
    add_manual_locked_constraints(
