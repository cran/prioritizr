...
* Add [targets](https://prioritizr.net/reference/targets.html) to a conservation planning problem to specify how much of each feature should ideally be represented  solutions.
...
First, we will load the _prioritizr_ package and some packages for working with spatial data.
...
print(p14)
```
* __Manual targets__: Targets are manually specified. This is only really recommended for advanced users or problems that involve multiple management zones. See the [_Management zones tutorial_](management_zones_tutorial.html) for more information on these targets.
...
that select planning units that share high connectivity values other planning units that are not selected by the solution. This function is used for asymmetric connectivities among planning units [based on @r38].
```{r}
...
p26 <- problem(sim_pu_raster, sim_features[[1:4]]) %>%
       add_min_set_objective() %>%
       add_relative_targets(0.1) %>%
...
open-source mixed integer programming solver that is part of the Computational Infrastructure for Operations Research (COIN-OR) project. Although formal benchmarks have yet to be completed, preliminary analyses suggest that it is
the fastest open-source solver available for generating prioritizations. It requires the _rcbc R_ package, which is currently only available on [GitHub](https://github.com/dirkschumacher/rcbc).
```{r}
...
eval_feature_representation_summary(p41, s41)
```
...
# calculate statistic
eval_connectivity_summary(p41, s41, data = cm)
```
...
# calculate statistic
eval_asym_connectivity_summary(p41, s41, data = acm)
```
...
```{r, fig.height = h, fig.width = w}
# formulate the problem
p42 <- problem(sim_pu_raster, sim_features) %>%
       add_min_set_objective() %>%
       add_relative_targets(0.1) %>%
       add_binary_decisions()
...
# plot solution
plot(s42, col = c("grey90", "darkgreen"), main = "Solution",
     xlim = c(-0.1, 1.1), ylim = c(-0.1, 1.1))
```
...
rwr42 <- eval_rare_richness_importance(p42, s42)

# plot rarity weighted richness scores
plot(rwr42, main = "Rarity weighted richness")
```
