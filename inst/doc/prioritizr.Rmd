...
# plot solution
plot(s33, col = c("grey90", "darkgreen"), main = "Solution",
...
# plot solution
plot(s34, col = c("grey90", "darkgreen"), main = "Solution",
     xlim = c(-0.1, 1.1), ylim = c(-0.1, 1.1))
...
cellStats(sim_pu_raster * s34, "sum")

# calculate how well features are represented in the solution
r34 <- feature_representation(p34, s34)
print(r34)
```
...
The _prioritizr R_ package offers two methods for calculating irreplaceability scores: replacement cost [`replacement_cost`; @r31] and rarity weighted richness [`rarity_weighted_richness`; @r33]. The replacement cost scores are generally recommended because they can (i) account for the cost of different planning units, (ii) account for multiple management zones, and (iii) identify truly irreplaceable planning units  (denoted with infinite values). Rarity weighted richness scores, on the other hand, cannot account for these three factors and consequently can provide misleading results about the importance of different planning units in a solution. The only advantage that rarity weighted richness scores have over replacement cost scores is that they are much, much faster to calculate (but see documentation for the replacement cost scores for information on reducing run times). As a general rule, we recommend using replacement cost scores for small and moderate sized problems (e.g. less than 30'000 planning units) and rarity weighted richness scores for large sized problems (e.g. more than 100'000 planning units). Below we will calculate irreplaceability scores using these two approaches.

```{r, fig.width = 6.5, fig.height = 2.5}
# calculate replacement cost scores and make the solver quiet
rc34 <- p34 %>%
        add_default_solver(verbose = FALSE) %>%
        replacement_cost(s34)

# calculate rarity weighted richness scores
rwr34 <- rarity_weighted_richness(p34, s34)

# plot irreplaceability scores
plot(stack(rc34, rwr34),
     main = c("replacement cost", "rarity weighted richness"))
```

Although rarity weighted richness scores can approximate replacement cost scores in certain conservation planning exercises, we can see that the rarity weighted richness scores provide completely difference results to the replacement cost scores in this case.
