...
cellStats(sim_pu_raster * s37, "sum")

# calculate how well features are represented in the solution
r37 <- feature_representation(p37, s37)
print(r37)
```
...
plot(rc37, main = "replacement cost")
```

```{r, fig.height = h, fig.width = w}
# calculate Ferrier irreplaceability scores and extract total score
fs37 <- ferrier_score(p37, s37)[["total"]]

# plot Ferrier scores
plot(fs37, main = "Ferrier scores")
```

```{r, fig.height = h, fig.width = w}
# calculate rarity weighted richness scores
rwr37 <- rarity_weighted_richness(p37, s37)

# plot replacement cost scores
plot(rwr37, main = "rarity weighted richness")
```
